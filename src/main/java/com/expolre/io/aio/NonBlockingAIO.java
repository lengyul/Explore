...
public class NonBlockingAIO {
	...
	public void client() throws IOException, InterruptedException, ExecutionException{
	   ...
	   /* ByteBuffer buffer = ByteBuffer.allocate(1024);
	    ...
	    if (avoid == null) { //返回null表示连接成功		
	    	while (asChannel.read(buffer).get() != -1) {
	    		buffer.flip();
	    		System.out.println("接收到服务端数据："+new String(buffer.array()));				
			}	
		}*/
	}
	...
	public void server() throws IOException, InterruptedException, ExecutionException{
		 ...
		 * 打开异步服务套接字通道
		 * open() 打开异步服务器套接字通道
		 * open(AsynchronousChannelGroup group) 指定异步通道组,用来"处理I/O事件"的线程池
		 */
		...
		while (true) {
             ...
             * 接收客户端连接，使用Future方式：
             * get方法是同步的
             * 使用Future操作I / O(accept/read/write)会阻塞到操作完成或者超时异常，需要考虑操作场景
             */
            ...
            if (asChannel.isOpen()) { //如果通道已经打开
            	//向客户端写入数据，Future方式
            	Future<Integer> result = asChannel.write(ByteBuffer.wrap("connecting successful!".getBytes()));
            	 ...
            	 * dst - 将数据读取到readBuffer
            	 * attachment - 要附加到I / O操作的对象 --> readBuffer
